...
def emit_image_store(op, ctx):
    ...
    gl_GlobalInvocationID.xy inline for coordinates rather than referencing
    a potentially undefined SSA variable from another pass.
    
    Uses imageLoad for READ_WRITE resources and texelFetch for READ samplers.
    """
    param = ctx['param']
    graph = ctx['graph']
    
    img = param(op.inputs[0])
    # Use inline coord expression to avoid cross-pass variable dependencies
...
def emit_image_load(op, ctx):
    ...
    we use inline gl_GlobalInvocationID.xy instead.
    """
    lhs = ctx['lhs']
    param = ctx['param']
    graph = ctx['graph']
...
def emit_sample(op, ctx):
    ...
    if uv_available:
        ...
        w, h = dispatch_size[0], dispatch_size[1]
